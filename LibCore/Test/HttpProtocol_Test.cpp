...
TEST(HttpProtocol , Test)
{
	...
	Assert_(p.m_nMode == Net::HttpProtocol::PARSE_MODE_HEADER);
	...
	Assert_(p.m_nMode == Net::HttpProtocol::PARSE_MODE_CONTENT);
	...
	Assert_(p.m_nMode == Net::HttpProtocol::PARSE_MODE_OK);
}